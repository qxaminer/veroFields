...
boolean squareVisible = true;                  // NEW flag to hide square //modelcom
...
float   wingSpeed   = 10;        // SLOWER than before (was 30)        //modelcom
...
void draw() {
  ...
  if (abs(grayNow - grayNext) < 1) {
    ...
    // spawn wing only if we just turned on connection AND square exists //modelcom
    if (newConnected && !connected && squareVisible) spawnWing();        //modelcom
  ...
  if (connected && squareVisible) {                                     //modelcom
  ...
  if (wingActive) {
    ...
    if (wingPos.dist(wingTarget) < wingSpeed) {   // contact achieved
      wingActive    = false;
      squareVisible = false;                      // hide the rectangle  //modelcom
    }
...
void initSquare() {
  ...
  squareVisible = true;                           // ensure visible on reset //modelcom
}
